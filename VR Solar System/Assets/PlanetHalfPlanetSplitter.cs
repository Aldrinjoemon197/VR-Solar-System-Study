...
public class PlanetHalfPlanetSplitter : MonoBehaviour
{
    ...
    public Vector3 tabletLocalOffset = new Vector3(1.35f, 0.85f, 0f);
    public float tabletScale = 1.25f;
    ...
    private void CreateTablet(Transform parent, List<LayerSpec> layers)
    {
        ...
        Bounds bounds = GetWorldBounds(parent);
        Vector3 cameraRight = Camera.main != null ? Camera.main.transform.right : Vector3.right;
        Vector3 tabletPosition =
            bounds.center +
            cameraRight.normalized * Mathf.Max(bounds.extents.x, bounds.extents.z, 0.5f) * tabletLocalOffset.x +
            Vector3.up * Mathf.Max(bounds.extents.y, 0.5f) * tabletLocalOffset.y;

        tablet.transform.position = tabletPosition;
        tablet.transform.rotation = Quaternion.identity;
        tablet.transform.localScale = Vector3.one * Mathf.Max(tabletScale, bounds.size.magnitude * 0.18f);
        ...
        TextMesh text = PlanetSplitMeshUtility.CreateText("Tablet Text", tablet.transform, new Vector3(-0.43f, 0.22f, -0.04f), 0.030f, TextAnchor.UpperLeft);

        string body = planetKind.ToString().ToUpper() + " LAYERS\n";

        for (int i = 0; i < layers.Count; i++)
        {
            body += layers[i].label + ": " + layers[i].description + "\n";
        }
    ...
    private Bounds GetWorldBounds(Transform parent)
    {
