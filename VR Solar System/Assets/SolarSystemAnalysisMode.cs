...
public class SolarSystemAnalysisMode : MonoBehaviour
{
    ...
    [Header("Auto Scene Additions")]
    [Tooltip("ON = automatically creates a procedural asteroid belt between Mars and Jupiter.")]
    public bool autoCreateAsteroidBelt = true;

    [Tooltip("ON = automatically creates ambient and action sounds.")]
    public bool autoCreateProceduralAudio = true;
    ...
    [Tooltip("ON = when A-B is already selected in Distance Mode, nearby third planets are preferred. OFF gives freer control of the two selected planets.")]
    public bool stronglyPreferThirdPlanetWhenDistancePairFull = false;
    ...
    [Tooltip("ON = makes Saturn's ring unlit/transparent so the flat plane does not catch ugly light.")]
    public bool improveSaturnRingVisual = true;

    [Tooltip("Makes the Saturn ring image look thicker/wider.")]
    public float saturnRingVisualScaleMultiplier = 1.28f;
    ...
    [Tooltip("ON = right thumbstick turns the view left/right while in analysis mode.")]
    public bool rightThumbstickRotatesCamera = true;

    [Tooltip("Old up/down camera target kept for saved scenes. Analysis mode now ignores up/down look.")]
    ...
    [Tooltip("Old up/down look speed kept for saved scenes. Analysis mode now ignores up/down look.")]
    ...
    [Tooltip("Old setting kept for saved scenes. Analysis mode now turns left/right only.")]
    public bool cameraRotationDominantAxisOnly = true;

    [Header("Planet Body Push")]
    [Tooltip("ON = planets feel solid in Analysis Mode by sliding the player sideways instead of letting planets pass through.")]
    public bool pushPlayerAwayFromPlanets = true;

    [Tooltip("Extra space between the player and the planet surface before the sliding push starts.")]
    public float planetPushExtraSpace = 0.65f;

    [Tooltip("Largest sideways push in one frame. Lower feels smoother; higher escapes faster.")]
    public float planetPushMaxStep = 0.22f;

    [Tooltip("Slightly enlarges the visual planet radius for the push zone.")]
    public float planetPushRadiusMultiplier = 1.04f;
    ...
    public float moonVisualSizeMultiplier = 0.065f;
    ...
    private TextMesh compareDifferencesText;
    private TextMesh compareSimilaritiesText;
    ...
    private readonly string[] compareGasLabels = new string[] { "CO2", "O2", "N2", "CH4", "H2", "He/Ar" };
    ...
    private bool saturnRingVisualFixed;
    private float nextIdentifierTextScanTime;

    void Start()
    {
        ApplyRequestedVisualTuning();
        ...
        if (autoCreateProceduralAudio)
        {
            ProceduralSolarSystemAudio.Ensure();
        }

        if (autoCreateAsteroidBelt)
        {
            EnsureAsteroidBelt();
        }
        ...
        FixSaturnRingVisuals();
        DisableIdentifierTextLabels();
    }

    void ApplyRequestedVisualTuning()
    {
        jupiterVisualMoonCount = Mathf.Max(jupiterVisualMoonCount, 34);
        saturnVisualMoonCount = Mathf.Max(saturnVisualMoonCount, 48);
        maxVisualMoons = Mathf.Max(maxVisualMoons, 48);
        moonVisualSizeMultiplier = Mathf.Min(moonVisualSizeMultiplier, 0.065f);
        giantPlanetMoonOrbitRingSpacingMultiplier = Mathf.Min(giantPlanetMoonOrbitRingSpacingMultiplier, 0.36f);
    }

    void EnsureAsteroidBelt()
    {
        AsteroidBeltGenerator generator = FindObjectOfType<AsteroidBeltGenerator>();

        if (generator != null)
        {
            if (generator.sun == null)
            {
                generator.sun = sun;
            }
        ...
        GameObject beltObject = new GameObject("Asteroid Belt Generator");
        generator = beltObject.AddComponent<AsteroidBeltGenerator>();
        generator.sun = sun;
    }
    ...
    void UpdateAnalysisFlightMovement()
    {
        ...
        ProceduralSolarSystemAudio.Ensure().SetPlayerThruster(true, move.magnitude);
    }
    ...
    void UpdatePlanetBodyPush()
    {
        if (!pushPlayerAwayFromPlanets)
        {
            return;
        }

        Transform rig = GetPlayerRigToMove();
        Transform bodyReference = flightDirectionReference != null ? flightDirectionReference : null;

        if (bodyReference == null && Camera.main != null)
        {
            bodyReference = Camera.main.transform;
        }

        if (rig == null || bodyReference == null)
        {
            return;
        }

        Transform[] allTransforms = FindObjectsOfType<Transform>();
        HashSet<Transform> checkedPlanets = new HashSet<Transform>();
        Vector3 totalPush = Vector3.zero;

        foreach (Transform t in allTransforms)
        {
            if (t == null || !IsKnownPlanetName(t.name) || checkedPlanets.Contains(t))
            {
                continue;
            }

            checkedPlanets.Add(t);

            float pushRadius = GetPlanetVisualRadius(t) * planetPushRadiusMultiplier + planetPushExtraSpace;
            Vector3 flatAway = bodyReference.position - t.position;
            flatAway.y = 0f;

            float distance = flatAway.magnitude;
            ...
            if (distance < 0.001f)
            {
                flatAway = Vector3.Cross(Vector3.up, bodyReference.forward);

                if (flatAway.sqrMagnitude < 0.001f)
                {
                    flatAway = bodyReference.right;
                }
            ...
            totalPush += flatAway.normalized * (pushRadius - distance);
        }
        ...
        Vector3 pushStep = Vector3.ClampMagnitude(totalPush, Mathf.Max(0.01f, planetPushMaxStep));
        CharacterController controller = rig.GetComponent<CharacterController>();

        if (controller != null)
        {
            controller.Move(pushStep);
        }
        else
        {
            rig.position += pushStep;
        }
    }
    ...
    AnalysisButton RaycastMenuButtonByPanelArea()
    {
        ...
        if (currentPage == AnalysisPage.Compare &&
            ...
            x >= -0.145f && x <= 0.145f && y >= -0.160f && y <= -0.100f)
        {
    ...
    Transform RaycastPlanet()
    {
        ...
        Transform bestDirectPlanet = null;
        float bestDirectDistance = Mathf.Infinity;
        ...
        if (directHits != null)
        {
            ...
            {
                if (hit.collider == null || hit.collider.transform.root == playerRoot)
                {
                    continue;
                }
                ...
                if (planet != null && hit.distance < bestDirectDistance)
                {
                    bestDirectDistance = hit.distance;
                    bestDirectPlanet = planet;
                }
            }
        }

        if (bestDirectPlanet != null)
        {
            return bestDirectPlanet;
        }

        RaycastHit[] sphereHits = Physics.SphereCastAll(
            ray.origin,
            Mathf.Min(planetSelectionSphereRadius, 0.45f),
            ray.direction,
            analysisRayDistance,
            planetHitLayers,
            QueryTriggerInteraction.Ignore
    ...
    Transform RaycastDirectPlanet()
    {
        ...
        foreach (RaycastHit hit in directHits)
        {
            ...
            if (planet != null && hit.distance < bestDistance)
            {
    ...
    void UpdateDistancePlanetMovement()
    {
        ...
        {
            Transform aimedPlanet = RaycastDirectPlanet();

            if (aimedPlanet == distancePlanetA || aimedPlanet == distancePlanetB)
            {
                activeMovePlanet = aimedPlanet;
            }
        }
    ...
    void ShowCompareText()
    {
        ...
        List<string> similarities = BuildSimilarityItems(a, b);
        List<string> differences = BuildDifferenceItems(a, b);
        ...
        ShowCompareColumnText(differences, similarities);
    }

    string BuildSimilarities(PlanetInfo a, PlanetInfo b)
    {
        return JoinShort(BuildSimilarityItems(a, b), 3);
    }

    string BuildDifferences(PlanetInfo a, PlanetInfo b)
    {
        return JoinShort(BuildDifferenceItems(a, b), 4);
    }

    List<string> BuildSimilarityItems(PlanetInfo a, PlanetInfo b)
    {
        ...
        {
            sims.Add("- Both have air");
        }

        if (a.hasMoons && b.hasMoons)
        {
            sims.Add("- Both have moons");
        }

        if (IsInnerRockyPlanet(a.displayName) && IsInnerRockyPlanet(b.displayName))
        {
            sims.Add("- Both inner planets");
            sims.Add("- Both have ground");
        }

        if (IsOuterGiantPlanet(a.displayName) && IsOuterGiantPlanet(b.displayName))
        {
            sims.Add("- Both outer planets");
            sims.Add("- Both very large");
        }

        if (NeedsSpacecraftSurface(a.displayName) && NeedsSpacecraftSurface(b.displayName))
        {
            sims.Add("- Unsafe to stand");
        }

        if (sims.Count == 1)
        {
            sims.Add("- Both can move");
            sims.Add("- Distance measurable");
        }
    ...
    List<string> BuildDifferenceItems(PlanetInfo a, PlanetInfo b)
    {
        List<string> diffs = new List<string>();

        if (a.displayName == b.displayName)
        {
            diffs.Add("Same planet selected.");
            return diffs;
        }

        AddPlanetTraitLine(diffs, a);
        diffs.Add("");
        AddPlanetTraitLine(diffs, b);

        return diffs;
    }

    void AddPlanetTraitLine(List<string> lines, PlanetInfo info)
    {
        List<string> traits = GetUniquePlanetTraits(info.displayName);
        lines.Add(info.displayName);

        for (int i = 0; i < traits.Count && i < 2; i++)
        {
            lines.Add("- " + traits[i]);
        }
    }

    string BuildCompareColumns(List<string> differences, List<string> similarities)
    {
        string result = "DIFFERENCES             SIMILARITIES\n";
        int rows = Mathf.Max(differences.Count, similarities.Count);

        for (int i = 0; i < rows; i++)
        {
            string left = i < differences.Count ? differences[i] : "";
            string right = i < similarities.Count ? similarities[i] : "";

            result += PadColumn(ShortenForColumn(left, 20), 24) + ShortenForColumn(right, 20);
    ...
    void ShowCompareColumnText(List<string> differences, List<string> similarities)
    {
        if (compareDifferencesText == null || compareSimilaritiesText == null)
        {
            return;
        }

        compareDifferencesText.gameObject.SetActive(true);
        compareSimilaritiesText.gameObject.SetActive(true);
        compareDifferencesText.text = "DIFFERENCES\n" + JoinLines(differences);
        compareSimilaritiesText.text = "SIMILARITIES\n" + JoinLines(similarities);
    }

    void HideCompareColumnText()
    {
        if (compareDifferencesText != null)
        {
            compareDifferencesText.gameObject.SetActive(false);
        }

        if (compareSimilaritiesText != null)
        {
            compareSimilaritiesText.gameObject.SetActive(false);
        }
    }
    ...
    string JoinLines(List<string> lines)
    {
        ...
        for (int i = 0; i < lines.Count; i++)
        {
            ...
            result += ShortenForColumn(lines[i], 22);
        }

        return result;
    }

    void LimitList(List<string> items, int maxCount)
    {
    ...
    string ShortenForColumn(string text, int maxLength)
    {
    ...
    string PadColumn(string text, int width)
    {
        if (text == null)
        {
            text = "";
    ...
    bool IsInnerRockyPlanet(string planetName)
    {
        return planetName == "Mercury" || planetName == "Venus" || planetName == "Earth" || planetName == "Mars";
    }

    bool IsOuterGiantPlanet(string planetName)
    {
        return planetName == "Jupiter" || planetName == "Saturn" || planetName == "Uranus" || planetName == "Neptune";
    }

    bool NeedsSpacecraftSurface(string planetName)
    {
        return planetName == "Venus" || planetName == "Jupiter" || planetName == "Saturn" || planetName == "Uranus" || planetName == "Neptune";
    }

    List<string> GetUniquePlanetTraits(string planetName)
    {
        List<string> traits = new List<string>();

        if (planetName == "Mercury")
        {
            traits.Add("smallest planet");
            traits.Add("almost no air");
            traits.Add("large iron core");
        }
        else if (planetName == "Venus")
        {
            traits.Add("hottest surface");
            traits.Add("very thick air");
            traits.Add("acid cloud layer");
        }
        else if (planetName == "Earth")
        {
            traits.Add("liquid water");
            traits.Add("life-friendly air");
            traits.Add("one large Moon");
        }
        else if (planetName == "Mars")
        {
            traits.Add("red dusty surface");
            traits.Add("very thin air");
            traits.Add("two small moons");
        }
        else if (planetName == "Jupiter")
        {
            traits.Add("largest planet");
            traits.Add("giant red storm");
            traits.Add("strong gravity");
        }
        else if (planetName == "Saturn")
        {
            traits.Add("bright rings");
            traits.Add("lowest density");
            traits.Add("Titan has air");
        }
        else if (planetName == "Uranus")
        {
            traits.Add("spins sideways");
            traits.Add("blue-green color");
            traits.Add("ice giant");
        }
        else if (planetName == "Neptune")
        {
            traits.Add("fastest winds");
            traits.Add("deep blue color");
            traits.Add("Triton orbits backward");
        }
        else
        {
            traits.Add("unknown");
    ...
    void BuildHologram()
    {
        ...
        compareShowChartsButton = CreateButton("AnalysisButton_GasCharts", "GAS CHARTS", compareRoot.transform, new Vector3(0f, -0.130f, -0.020f));
        compareShowChartsButton.SetActive(false);

        compareDifferencesText = CreateText("Compare Differences Column", compareRoot.transform, new Vector3(-0.270f, 0.085f, -0.020f), 0.0041f);
        compareDifferencesText.fontSize = 36;
        compareDifferencesText.anchor = TextAnchor.UpperLeft;
        compareDifferencesText.alignment = TextAlignment.Left;
        compareDifferencesText.gameObject.SetActive(false);

        compareSimilaritiesText = CreateText("Compare Similarities Column", compareRoot.transform, new Vector3(0.030f, 0.085f, -0.020f), 0.0041f);
        compareSimilaritiesText.fontSize = 36;
        compareSimilaritiesText.anchor = TextAnchor.UpperLeft;
        compareSimilaritiesText.alignment = TextAlignment.Left;
        compareSimilaritiesText.gameObject.SetActive(false);
    ...
    float[] GetAtmospherePercentages(Transform planet)
    {
        ...
        if (n.Contains("mercury")) return new float[] { 0f, 0f, 0f, 0f, 0f, 0f };
        if (n.Contains("venus"))   return new float[] { 96.5f, 0f, 3.5f, 0f, 0f, 0f };
        if (n.Contains("earth"))   return new float[] { 0.04f, 20.95f, 78.08f, 0.0002f, 0f, 0.93f };
        if (n.Contains("mars"))    return new float[] { 95.1f, 0.16f, 2.59f, 0.0004f, 0f, 1.94f };
        if (n.Contains("jupiter")) return new float[] { 0f, 0f, 0f, 0.3f, 89.8f, 9.9f };
        if (n.Contains("saturn"))  return new float[] { 0f, 0f, 0f, 0.4f, 96.3f, 3.25f };
    ...
    void BuildMoonVisualsForPlanet(Transform planet)
    {
        ...
        int moonsPerRing = IsGiantPlanetMoonCluster(planet) ? 12 : 8;
    ...
    int GetVisualMoonCountForPlanet(Transform planet, int knownMoonCount)
    {
        ...
        // Compact visual ratio: giant planets get many small clustered moons.
    ...
    bool IsGiantPlanetMoonCluster(Transform planet)
    {
        string n = PlanetDisplayName(planet).ToLower();
        return n.Contains("jupiter") || n.Contains("saturn");
    }

    void FixSaturnRingVisuals()
    {
        if (!improveSaturnRingVisual || saturnRingVisualFixed)
        {
            return;
        }

        Transform ring = saturnRingObject != null ? saturnRingObject : FindSaturnRingObject();
        ...
        saturnRingObject = ring;
        ApplySaturnRingVisualFix(ring);
        saturnRingVisualFixed = true;
    }

    Transform FindSaturnRingObject()
    {
        ...
        foreach (Transform t in transforms)
        {
            ...
            if (n.Contains("saturnring") || (n.Contains("saturn") && n.Contains("ring")))
            {
                return t;
            }
        }
    ...
    void ApplySaturnRingVisualFix(Transform ring)
    {
        if (saturnRingVisualScaleMultiplier > 0.01f)
        {
            ring.localScale = new Vector3(
                ring.localScale.x * saturnRingVisualScaleMultiplier,
                ring.localScale.y * saturnRingVisualScaleMultiplier,
                ring.localScale.z
            );
        }
        ...
        foreach (Renderer renderer in renderers)
        {
            ...
            Material ringMaterial = CreateSaturnRingMaterial(source);
            renderer.material = ringMaterial;
        }

        Collider[] colliders = ring.GetComponentsInChildren<Collider>(true);
    ...
    Material CreateSaturnRingMaterial(Material source)
    {
        ...
        Color color = new Color(1f, 0.92f, 0.72f, 0.88f);

        if (source != null)
        {
            color = GetMaterialColor(source, color);
        }
    ...
    Texture GetMaterialTexture(Material material)
    {
        if (material == null)
        {
            return null;
        ...
        string[] textureNames = { "_BaseColorMap", "_UnlitColorMap", "_MainTex", "_BaseMap" };
    ...
    {
        string[] colorNames = { "_BaseColor", "_UnlitColor", "_Color" };
        ...
        foreach (string colorName in colorNames)
        {
            if (material.HasProperty(colorName))
            {
                ...
                color.a = Mathf.Clamp(color.a, 0.75f, 0.95f);
                return color;
            }
        }
    ...
    void DisableIdentifierTextLabels()
    {
        if (Time.unscaledTime < nextIdentifierTextScanTime)
        {
            return;
        }

        nextIdentifierTextScanTime = Time.unscaledTime + 1f;

        TextMesh[] texts = FindObjectsOfType<TextMesh>();
        ...
        foreach (TextMesh text in texts)
        {
            ...
            string value = text.text != null ? text.text.ToLower() : "";

            if (objectName.Contains("identifier") || value == "identifier" || value == "identifier:")
            {
