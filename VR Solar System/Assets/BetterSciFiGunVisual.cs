...
public class BetterSciFiGunVisual : MonoBehaviour
{
    ...
    [Tooltip("Tilts the gun and laser slightly downward when the controller is held straight.")]
    public float downwardAimAngle = -10f;

    [Header("Solid Feel")]
    public bool makeGunSolid = true;
    public Vector3 gunSolidColliderCenter = new Vector3(0f, -0.035f, 0.095f);
    public Vector3 gunSolidColliderSize = new Vector3(0.13f, 0.22f, 0.48f);

    [Tooltip("Moves the visible gun slightly away from the opposite glove when they overlap.")]
    public bool visuallySeparateFromOtherGlove = true;
    public float gloveSeparationRadius = 0.38f;
    public float gloveSeparationStrength = 1.15f;
    public float maxGloveSeparationOffset = 0.18f;
    ...
    private Vector3 baseGunLocalPosition;
    ...
    public void BuildGun()
    {
        ...
        gunVisualRoot = gunRoot.transform;
        baseGunLocalPosition = gunLocalPosition;
        gunRoot.transform.SetParent(transform);
        gunRoot.transform.localPosition = gunLocalPosition;
        gunRoot.transform.localRotation = Quaternion.Euler(gunLocalRotation + new Vector3(downwardAimAngle, 0f, 0f));
    ...
    void LateUpdate()
    {
        if (!visuallySeparateFromOtherGlove || gunVisualRoot == null)
        {
            return;
        }

        if (!TryFindOtherGloveCenter(out Vector3 otherGloveCenter))
        {
            gunVisualRoot.localPosition = Vector3.Lerp(gunVisualRoot.localPosition, baseGunLocalPosition, Time.deltaTime * 14f);
            return;
        }

        Vector3 away = gunVisualRoot.position - otherGloveCenter;
        ...
        if (distance >= gloveSeparationRadius)
        {
            gunVisualRoot.localPosition = Vector3.Lerp(gunVisualRoot.localPosition, baseGunLocalPosition, Time.deltaTime * 14f);
            return;
        }

        float overlap = gloveSeparationRadius - distance;
        float pushAmount = Mathf.Min(maxGloveSeparationOffset, overlap * gloveSeparationStrength);
        Vector3 pushedLocalOffset = transform.InverseTransformVector(away.normalized * pushAmount);
        Vector3 targetLocalPosition = baseGunLocalPosition + pushedLocalOffset;
        gunVisualRoot.localPosition = Vector3.Lerp(gunVisualRoot.localPosition, targetLocalPosition, Time.deltaTime * 28f);
    }

    bool TryFindOtherGloveCenter(out Vector3 center)
    {
        ...
        foreach (Renderer renderer in renderers)
        {
            ...
            if (!n.Contains("glove") && !n.Contains("hand") && !parentName.Contains("glove") && !parentName.Contains("hand"))
            {
                continue;
            }

            float distance = Vector3.Distance(gunVisualRoot.position, renderer.bounds.center);
    ...
    Transform FindOrCreateMuzzle()
    {
        ...
        if (existing != null)
        {
            ...
            existing.localRotation = Quaternion.Euler(muzzleLocalRotation + new Vector3(downwardAimAngle, 0f, 0f));
        ...
        muzzle.transform.localRotation = Quaternion.Euler(muzzleLocalRotation + new Vector3(downwardAimAngle, 0f, 0f));
        muzzle.transform.localScale = Vector3.one;

        return muzzle.transform;
    }

    void AddGunSolidCollider(GameObject gunRoot)
    {
        if (!makeGunSolid || gunRoot == null)
        {
            return;
        }

        BoxCollider collider = gunRoot.GetComponent<BoxCollider>();

        if (collider == null)
        {
            collider = gunRoot.AddComponent<BoxCollider>();
        }

        collider.isTrigger = false;
        collider.center = gunSolidColliderCenter;
        collider.size = gunSolidColliderSize;

        Rigidbody rb = gunRoot.GetComponent<Rigidbody>();

        if (rb == null)
        {
            rb = gunRoot.AddComponent<Rigidbody>();
        }
