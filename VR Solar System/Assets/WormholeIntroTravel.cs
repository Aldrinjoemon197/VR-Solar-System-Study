...
public class WormholeIntroTravel : MonoBehaviour
{
    ...
    [Tooltip("Optional landing point for ExplorerSpaceship. If empty, the script uses ShipSolarSystemLandingPoint by name, then falls back to Landing Point.")]
    public Transform shipLandingPoint;
    ...
    [Header("ALWAYS VISIBLE BLUE WORMHOLE")]
    ...
    public Color wormholeBlueColor = new Color(0f, 0.55f, 1f, 1f);
    ...
    private SpaceshipThirdPersonController activeShipController;
    private bool currentTravelIncludesShip;
    ...
    void Start()
    {
        ...
        if (shipLandingPoint == null)
        {
            GameObject foundShipLanding = GameObject.Find("ShipSolarSystemLandingPoint");

            if (foundShipLanding != null)
            {
                shipLandingPoint = foundShipLanding.transform;
            }
        }
    ...
    public void StartWormhole()
    {
        ...
        travelling = true;
        travelTimer = 0f;
        activeShipController = FindActiveShipController();
        Transform finalShipLandingPoint = shipLandingPoint != null ? shipLandingPoint : landingPoint;
        currentTravelIncludesShip =
            activeShipController != null &&
            activeShipController.BeginShipWormholeTravel(finalShipLandingPoint);
    ...
    void UpdateTravelMovement()
    {
        ...
        if (currentTravelIncludesShip && activeShipController != null)
        {
            activeShipController.SetShipWormholeProgress(t);
        }

        if (!currentTravelIncludesShip && moveRigDuringWormhole && playerRigToMove != null && landingPoint != null)
        {
    ...
    void EndWormhole()
    {
        if (currentTravelIncludesShip && activeShipController != null)
        {
            activeShipController.CompleteShipWormholeTravel();
        }

        if (!currentTravelIncludesShip && forceRigExactlyToLandingPoint && playerRigToMove != null && landingPoint != null)
        {
        ...
        travelling = false;
        currentTravelIncludesShip = false;
        activeShipController = null;
    ...
    void CreateWormholeLockedToCamera()
    {
        ...
        wormholeRoot = new GameObject("Always Visible Blue Wormhole");
        ...
        for (int i = 0; i < ringCount; i++)
        {
            ...
            Material mat = CreateMaterial(wormholeBlueColor);
            lr.material = mat;
    ...
    SpaceshipThirdPersonController FindActiveShipController()
    {
        SpaceshipThirdPersonController[] controllers = FindObjectsOfType<SpaceshipThirdPersonController>();

        for (int i = 0; i < controllers.Length; i++)
        {
            SpaceshipThirdPersonController controller = controllers[i];

            if (controller != null && controller.enabled && controller.IsInShipMode)
            {
                return controller;
            }
        }
    ...
    void OnDisable()
    {
        if (currentTravelIncludesShip && activeShipController != null)
        {
            activeShipController.CancelShipWormholeTravel();
        }
